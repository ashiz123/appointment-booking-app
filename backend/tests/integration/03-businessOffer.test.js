...
describe('Test create business offer', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.error(
        'Error in before each, running test to create business offer',
        error
      );
      throw error;
    }
...
describe('Failure updating and deleting business offer', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const newBusinessOffer = await global.db
        .collection('business_offer')
        .insertOne(businessOfferData);
  ...
  it('should response the error of no any data provided', async () => {
    ...
    const res = await request(app)
      .put(`/business/offer/update/${businessOfferId}`)
      .set('Accept', 'application/json')
  ...
  it('should not allow delete the business offer to unauthorised user', async () => {
    const res = await request(app)
      .delete(`/business/offer/delete/${businessOfferId}`)
      .set('Accept', 'application/json')
      .set('Authorization', 'Bearer valid_token 123');
